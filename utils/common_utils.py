...
def diff(img1,img2,coreSize = (5,5)):
    img1 = img1.resize(coreSize, Image.ANTIALIAS)
    img2 = img2.resize(coreSize, Image.ANTIALIAS)
    img1_np = pil_to_np(img1)
    img2_np = pil_to_np(img2)
    mse = mean_squared_error(img1_np[0],img2_np[0])\
         +mean_squared_error(img1_np[1],img2_np[1])\
         +mean_squared_error(img1_np[2],img2_np[2])
    return mse

def pil_to_thumbnail_np(img,coreSize = (5,5)):
    thumbnail = img.resize(coreSize, Image.ANTIALIAS)
    thumb_np = pil_to_np(thumbnail)
    return thumb_np

def centeredCrop2(img,scale=1):
   width =  np.size(img,1)
   height =  np.size(img,0)
   w = min(width,height)
   w = int(np.floor(w * scale))
   return centeredCrop(img,w,w)

def centeredCrop3(img,scale=(1,1)):
   width =  np.size(img,1)
   height =  np.size(img,0)
   w = min(width,height)
   w = int(np.floor(w * scale[0]))
   h = int(np.floor(w * scale[1]))
   return centeredCrop(img,w,h)

def centeredCrop(img, new_width, new_height):
   width =  np.size(img,1)
   height =  np.size(img,0)
